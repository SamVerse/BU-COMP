    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track your submissions and explore new events</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Total Submissions" value={stats.totalSubmissions} icon="📤" color="blue" />
        <StatCard title="Events Participated" value={stats.eventsParticipated} icon="📅" color="green" />
        <StatCard title="Under Review" value={stats.underReview} icon="👀" color="yellow" />
      </div>
  ...
  return (
    <Card padding="default" hover>
      <div className="flex items-center justify-between">
        ...
        <div className={`w-12 h-12 rounded-lg ${colorStyles[color]} flex items-center justify-center text-2xl`}>
          {icon}
        </div>
      </div>
    </Card>
    ...
    <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
      <div className="flex-1">
        ...
        <p className="text-sm text-gray-500 mt-1">
          Deadline: {new Date(event.deadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })} ({event.daysLeft} days left)
        </p>
      </div>
